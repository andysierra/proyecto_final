...
<div class="card-body">
    <h2 class="segan mb-4">Solicitudes</h2>
...
$(document).ready(function(){
    ...
    function search() {
        $('#btnPdf').attr('href', "indexPDF.php?pid=<?= base64_encode('presentation/frame/pdf/PdfTableRequisition.php')?>&filter="+$('#txfSearch').val());
        
        let chartRoute = "indexAjax.php?pid=<?= base64_encode('presentation/frame/ajax/AjaxChartRequisition.php')?>&filter="+$('#txfSearch').val();
        $('#charts').load(chartRoute);
        
        let route = "indexAjax.php?pid=<?= base64_encode('presentation/frame/ajax/AjaxTableRequisition.php')?>&filter="+$('#txfSearch').val();
        $('#rows').load(route);
    }
